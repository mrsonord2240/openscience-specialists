...
- Do not use for searching non-US trial registries (e.g., EU CTR, ISRCTN, ChiCTR) — the API only covers ClinicalTrials.gov.
- Do not use for downloading full study protocols or CSR documents (the API provides structured metadata only).
- Do not use for real-time patient matching in clinical settings without verification of current recruitment status.
- Do not use when offline access is required (the API requires internet connectivity).
...
This skill accepts requests that match the documented purpose of `clinicaltrials-database` and include enough context to complete the workflow safely.

Do not continue the workflow when the request is out of scope, missing a critical input, or would require unsupported assumptions. Instead respond:

> `clinicaltrials-database` only handles its documented workflow. Please provide the missing required inputs or switch to a more suitable skill.
