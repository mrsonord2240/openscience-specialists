...
- The task requires de novo variant calling or genotype imputation (use specialized bioinformatics pipelines)
- The user needs fine-mapping or colocalization analysis (use dedicated tools)
- The query is about gene expression rather than genetic associations (use GTEx or expression databases)
...
2. **Query associations via API:**
   ...
   **If API returns non-200 status:** Check EFO ID validity, retry with alternative trait IDs, or fall back to web interface search.
...
> **🔍 Checkpoint 1:** Present the filtered association results to the user. Confirm whether to proceed with cross-referencing or expand the search criteria.
...
This skill accepts requests that match the documented purpose of `gwas-database` and include enough context to complete the workflow safely.

Do not continue the workflow when the request is out of scope, missing a critical input, or would require unsupported assumptions. Instead respond:

> `gwas-database` only handles its documented workflow. Please provide the missing required inputs or switch to a more suitable skill.
